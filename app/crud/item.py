...
def create_item(db: Session, item: schema.ItemCreate, current_user):
    db_item = models.Item(owner_id=current_user.id, **item.dict())
    db.add(db_item)
    db.commit()
    db.refresh(db_item)
...
def update_item(db: Session, id: int, item_dict: schema.ItemCreate, current_user):
